...
from visframe import make_frame, make_frame_clean
...
def make_skeleton_video(filename, org=False, fps=30, directory="data/joint_videos/"):
    '''
    Input:
    ...
    if org:
        ...
        make_frame(data, joints=17, output_filename=output_file)
    else:
        output_file = directory + f"{flabel}.mp4"
        make_frame_clean(data, joints=17, output_filename=output_file, fps=fps)
...
def split_pose(filename, s=5, fps=10):
    ...
    audio = wav.read(f"data/audio/{flabel}.wav")[1]
...
def normalize_pose(filename):
    ...
    max_size = 0 #max max min min mean mean
    prev = None
    newdata = []
    ...
    for i, d in enumerate(data):
        ...
        # TODO: pose-nose for all frames, nose = nose - prevnose
        if i == 0:
            prev = pose
            ...
            prev = pose
        dancer_size = max(abs(rankle-nose)[1], abs(lankle-nose)[1])
        max_size = max(max_size, dancer_size)
...
'''
지금문제가...
5초나눈것들 중간거 비디오로만들면 망했어
initial pose가 필요해!
- normalize after split
- every start of the 5s should be initial pose (split할떄 바로전 프레임을 맞춰서 주는거지...)
- normalize 할떄 이전프레임과 차이가 아닌 nose와 차이를! 각 nose값은 이전nose와 차이를
'''
